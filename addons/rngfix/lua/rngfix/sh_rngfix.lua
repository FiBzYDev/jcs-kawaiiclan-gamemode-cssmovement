...
local GetSpeedCap = function(ply) return 30.0 end

local function InitPlayerArrayValues(ply)
	g_iTick[ply] = 0
	...
	ply.RNGFix = ply.RNGFix or {}
	if SERVER then RNGFix.BoosterFixPlayerInit(ply) end
end

local function DeletePlayerArrayValues(ply)
	g_iTick[ply] = nil
...
local function Duck(ply, origin)
	...
	return g_vecMins, (nextDucking and g_vecMaxsDucked or g_vecMaxsUnducked), origin
end
...
local function PreventCollision(ply, mv, origin, collisionPoint, velocity_tick)
	Debug.Laser(ply, origin, collisionPoint, 1)
	...
	Debug.Laser(ply, origin, newOrigin, 2)
	local adjustment = newOrigin - origin
	Debug.Msg(ply, "Moved: %.2f %.2f %.2f", adjustment.x, adjustment.y, adjustment.z)
...
local function SetVelocity(ply, velocity, mv, dontUseTeleportEntity)
	...
	Debug.Msg(ply, "SetVelocity: %.2f %.2f %.2f", velocity.x, velocity.y, velocity.z)
	-- print(ply:GetVelocity(), ply:GetAbsVelocity(), ply:GetBaseVelocity() - g_vLastBaseVelocity[ply])
...
local function TeleportEntity(ent, pos, ang, vel)
	local solid = ent:GetSolidFlags()
	ent:AddSolidFlags(FSOLID_NOT_SOLID)

	if ang then
		ent:SetAngles(ang)
		if ent:IsPlayer() then ent:SetEyeAngles(ang) end
	end

	if vel then
		local absVel = GetInternalVariable("m_vecAbsVelocity")
		if absVel ~= vel then
			ply:RemoveEFlags(EFL_DIRTY_ABSVELOCITY)
			ply:SetSaveValue("m_vecAbsVelocity", vel)
		end
		ply:SetSaveValue("m_vecBaseVelocity", vector_origin)

		local parent = ent:GetMoveParent()
		if not IsValid(parent) then
			ply:SetSaveValue("m_vecVelocity", vel)
		else
			local new_vel = vel - parent:GetInternalVariable("m_vecAbsVelocity")
			new_vel:Rotate(parent:GetWorldTransformMatrix():GetAngles())
			ply:SetSaveValue("m_vecVelocity", new_vel)
		end
	end

	if pos then ent:SetPos(pos) end

	-- TODO: Complete TeleportEntity from baseentity.cpp
...
local function RunPreTickChecks(ply, mv, cmd)
	...
	-- This does noting unfinished code
	...
	if tr.Hit then
		local nrm, collisionPoint = tr.HitNormal, tr.HitPos

		if nrm.z ~= 1 and g_iLastCollisionTick[ply] < g_iTick[ply] - 1 then
			Debug.Msg(ply, "Collision predicted! (normal: %.3f %.3f %.3f)", nrm.x, nrm.y, nrm.z)
		end
			...
			if nrm.z < 1.0 and nrm.x*velocity.x + nrm.y*velocity.y < 0.0 then
				...
				if g_cvDownhill:GetBool() then
					...
					if newVelocity.x*newVelocity.x + newVelocity.y*newVelocity.y > velocity.x*velocity.x + velocity.y*velocity.y then
						...
						Debug.Msg(ply, "shouldDoDownhillFixInstead")
					end
				end

				if not shouldDoDownhillFixInstead then
					Debug.Msg(ply, "DO FIX: Uphill Incline")
		...
		if g_cvEdge:GetBool() and GetEdgeBugFixEnabled(ply) then
			...
			Debug.Msg(ply, "DO FIX: Edge Bug")
			Debug.Laser(ply, collisionPoint, tickEnd, 1)
...
local function ProcessMovementPre(ply, mv, cmd)
...
local function ProcessMovementPost(ply, mv)
	if not ply:Alive() then return end
	if ply:GetMoveType() ~= MOVETYPE_WALK then return end
	if CheckWater(ply) then return end
	...
	if SERVER then DoTelehopFix(ply, mv) end
end
